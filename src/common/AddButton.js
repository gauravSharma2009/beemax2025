    ...
    return (
        type && type === 'plus' ?
            !item.qty_added_in_cart || Number(item.qty_added_in_cart) === 0 ?
                <TouchableOpacity
                    onPress={item.in_stock === "0" || parseInt(item.inventory) < 1 ? null : handleAdd}
                    style={[styles.button, { backgroundColor: item.in_stock === "0" || parseInt(item.inventory) < 1 ? "#E5E5E5" : allCategoryPink, borderColor: allCategoryPink, borderWidth: 1 }, style, { maxHeight: 35 }]} onPress={handleAdd}>
